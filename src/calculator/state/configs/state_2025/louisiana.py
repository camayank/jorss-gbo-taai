"""Louisiana state tax calculator for tax year 2025."""
...
def get_louisiana_config() -> StateTaxConfig:
    return StateTaxConfig(
        state_code="LA",
        state_name="Louisiana",
        tax_year=2025,
        is_flat_tax=True,
        flat_rate=0.03,  # 3% flat rate (new for 2025 - converted from progressive)
        brackets=None,
        starts_from="federal_agi",
        standard_deduction={"single": 12500, "married_joint": 25000, "married_separate": 12500,
                           "head_of_household": 12500, "qualifying_widow": 25000},
        ...
        allows_federal_tax_deduction=False,  # Changed in 2025 reform
        social_security_taxable=False,
        pension_exclusion_limit=6000,
        military_pay_exempt=True,
        eitc_percentage=0.05,  # Louisiana EITC 5% of federal
        child_tax_credit_amount=0,
        has_local_tax=False,
    )


@register_state("LA", 2025)
class LouisianaCalculator(BaseStateCalculator):
    """Louisiana state tax calculator - flat 3% rate for 2025."""

    def __init__(self):
        super().__init__(get_louisiana_config())
    ...
    def calculate(self, tax_return: "TaxReturn") -> StateCalculationBreakdown:
        ...
        la_agi = federal_agi + additions - subtractions

        std_deduction = self.config.get_standard_deduction(filing_status)
        deduction_used = "standard"
        deduction_amount = std_deduction

        la_taxable_income = max(0.0, la_agi - deduction_amount)
        tax_before_credits = self.calculate_brackets(la_taxable_income, filing_status)

        credits = {}
        federal_eitc = self._calculate_federal_eitc_estimate(tax_return, filing_status)
        la_eitc = self.calculate_state_eitc(federal_eitc)
        if la_eitc > 0:
            credits["la_eitc"] = la_eitc
        ...
        return StateCalculationBreakdown(
            ...
            state_adjusted_income=la_agi, state_standard_deduction=std_deduction,
            state_itemized_deductions=0.0, deduction_used=deduction_used,
            ...
            state_taxable_income=la_taxable_income,
