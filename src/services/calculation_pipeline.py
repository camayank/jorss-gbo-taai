...
class PipelineStep(ABC):
    ...
    Each step processes the context and passes it to the next step.
    """
...
class PrepareStep(PipelineStep):
    ...
    Steps:
    - Load prior year return if available
    - Apply carryovers
...
class FederalCalculationStep(PipelineStep):
    ...
    Uses the FederalTaxEngine to compute:
    - Income tax
    - Self-employment tax
...
class StateCalculationStep(PipelineStep):
    ...
    Uses the StateTaxEngine for state-specific calculations.
    """
...
class OutputValidationStep(PipelineStep):
    ...
    def execute(self, context: PipelineContext) -> PipelineContext:
        ...
        if abs(expected_refund - breakdown.refund_or_owed) > 1:  # Allow $1 rounding
            context.add_warning(
...
class CalculationPipeline:
    """
    Orchestrates the tax calculation pipeline.
...
def create_pipeline(
    ...
    """
    Create a calculation pipeline with optional steps.
    ...
    Returns:
        Configured CalculationPipeline
    """
    steps = []
    ...
    steps.append(FederalCalculationStep())

    if include_state:
        steps.append(StateCalculationStep())

    if include_validation:
        steps.append(OutputValidationStep())

    return CalculationPipeline(steps)
