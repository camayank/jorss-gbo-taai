...
if _sentry_dsn:
    sentry_sdk.init(
        ...
        profiles_sample_rate=0.1 if _log_env in ("production", "prod") else 0.0,
...
# =============================================================================
# MIDDLEWARE CONFIGURATION (extracted to web/middleware_setup.py)
# =============================================================================
from web.middleware_setup import configure_middleware
_middleware_result = configure_middleware(app)
_csrf_secret_key = _middleware_result.get("csrf_secret_key")
...
# NOTE: Additional middleware (Correlation ID, RBAC v2, Tenant Isolation)
# now configured via web.middleware_setup.configure_middleware() above.
...
# =============================================================================
# EXTRACTED ROUTERS (from app.py monolith decomposition)
# =============================================================================
# Page routes (HTML page renderers, redirects, legal pages)
try:
    from web.routes.page_routes import router as _page_router
    app.include_router(_page_router)
    logger.info("Registered: Extracted Page Routes")
except ImportError as e:
    logger.debug(f"Extracted page routes not available: {e}")

# Chat routes (/api/chat)
try:
    from web.routes.chat_routes import router as _chat_router
    app.include_router(_chat_router)
    logger.info("Registered: Extracted Chat Routes")
except ImportError as e:
...
async def http_exception_handler(request: Request, exc: StarletteHTTPException):
    ...
    if exc.status_code in _error_templates:
        try:
            ...
            _dashboard_urls = {"admin": "/admin", "cpa": "/cpa/dashboard", "client": "/client"}
...
# =============================================================================
# PAGE ROUTES - Extracted to web/routes/page_routes.py
# =============================================================================
# All HTML page renderers (/, /landing, /intelligent-advisor, /dashboard,
# /admin, /guided, /results, etc.) are now in web.routes.page_routes.
# Registered via app.include_router() in the EXTRACTED ROUTERS section above.

# =============================================================================
# ROLE-BASED ROUTING HELPERS (used by page routes AND error handlers)
# =============================================================================
from web.constants import ADMIN_UI_ROLES as _ADMIN_UI_ROLES, CPA_UI_ROLES as _CPA_UI_ROLES, CLIENT_UI_ROLES as _CLIENT_UI_ROLES
...
# Page route handlers (/app, /app/workspace, /app/portal, /app/settings,
# /legacy/cpa/*, /cpa/settings/*, /cpa-landing, /for-cpas, /terms, /privacy,
# /cookies, /disclaimer, /client, /logout, /scenarios, /projections, /contact,
# /settings, /documents, /returns, /clients, /advisory-report-preview)
# => Extracted to web/routes/page_routes.py
...
# Page routes (/admin, /hub, /system-hub, /workflow, /workflow-hub)
# => Extracted to web/routes/page_routes.py
...
# Page routes (/smart-tax, /guided, /results) => Extracted to web/routes/page_routes.py

# =============================================================================
# CHAT ENDPOINT - Extracted to web/routes/chat_routes.py
# =============================================================================
# The /api/chat endpoint is now in web.routes.chat_routes.
# Registered via app.include_router() in the EXTRACTED ROUTERS section above.
...
# =============================================================================
# STARTUP / SHUTDOWN LIFECYCLE (extracted to web/startup.py)
# =============================================================================
from web.startup import register_lifecycle_events
...
# Interview/validation API and HTMX partials => web/routers/interview_api.py
# Scenario routes => web/routers/scenarios.py
# Entity comparison, retirement analysis, smart insights => web/routers/tax_tools.py
# Legacy advisor redirects => web/routes/page_routes.py
